...
Hermes should connect to ToolGate through a local MCP bridge, not through raw
CLI calls and not through MemoryGate directly.

## Boundary

- Hermes talks to the ToolGate MCP server.
...
configured control-plane state and exposes them as MCP tools for Hermes.
...
`research.search` is exposed to Hermes as `research_search`, while the original
...
When Hermes invokes one of those MCP tools, the bridge calls ToolGate's own
...
The bridge also exposes `toolgate_request_status` so Hermes can inspect pending
...
The same example is included at `integrations/mcp/toolgate.hermes.mcp.json`.
